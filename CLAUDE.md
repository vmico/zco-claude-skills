...
**Repository**: `_.ai-claude-skills`
**Parent Project**: YJ-ANNO-SG-Y2025 (Annotation Services Platform)
...
```
_.ai-claude-skills/
├── .claude/                          # Active Claude configuration (symlinked)
│   ├── hooks -> ../../_.ai-claude/.claude/hooks
│   ├── rules -> ../../_.ai-claude/.claude/rules
│   ├── skills -> ../../_.ai-claude/.claude/skills
...
│   ├── README.md                     # Plan management guide (630+ lines)
│   ├── plan.template.md              # Full plan template
│   ├── plan.template.minimal.md      # Minimal plan template
...
├── .claudeignore                     # Files excluded from Claude
├── CLAUDE.md                         # This file
...
priority: "p2:中:可纳入后续迭代计划"    # Priority enum
...
**Priority Values**:
- `p0:紧急:重要` - Urgent and important
- `p1:高:当前迭代/排期内重点解决` - High priority (current iteration)
- `p2:中:可纳入后续迭代计划` - Medium priority (default)
- `p3:低:可记录，待后续评估` - Low priority
- `p4:无:不影响当前迭代/排期` - No priority

**Markdown Content** (Required sections):
- 🎯 **Goal** - Core objective (1-2 sentences)
- 📋 **Detailed Requirements** - Feature description, special requirements, I/O specs
- ✅ **Verification Standards** - Completion criteria checklist
- 🧪 **Test Plan** - Unit tests, integration tests, manual tests
- 📚 **Reference Information** - Related docs, existing implementations
...
- `.claude/skills` → Custom skills (zco-*)
...
The parent project maintains coding standards in `_.ai-claude/.claude/rules/`:

- **Go Coding Standards** (`go/coding-standards.md`)
  - Comment conventions (`//` vs `//;` vs `//;@TODO:`)
  - Naming rules
  - Error handling
  - Concurrency patterns
  - Testing requirements (≥80% coverage)

- **Go Testing Rules** (`go/go-testing.md`)
  - Table-driven tests
  - Test organization
  - Mock strategies
  - Coverage requirements

These rules are symlinked into this repository's `.claude/rules/` directory.
...
The project uses a unique comment system:

| Prefix | Purpose | Example |
|--------|---------|---------|
| `//` | Standard code comments | `// CalculateTotal computes order total` |
| `//;` | Code logic explanation | `//;Small orders don't get discounts` |
| `//;@TODO:` | Pending tasks | `//;@TODO: Add email validation` |
| `//;@FIXME:` | Bugs to fix | `//;@FIXME: Redis timeout issue` |
| `//;@NOTE:` | Developer notes | `//;@NOTE: Consider concurrency` |
| `//;@HACK:` | Temporary solutions | `//;@HACK: Remove after API v2` |
| `//;@OPTIMIZE:` | Performance improvements | `//;@OPTIMIZE: Use batch query` |
| `//;@DEPRECATED:` | Deprecated code | `//;@DEPRECATED: Use V2 instead` |
...
- ✅ Linter checks pass (`golangci-lint`)

**For Python Code** (zco_claude_init.py):
- ✅ Use `##;` prefix for logic explanations
...
4. ✅ Ensure `name` field matches directory name
...
**Solutions**:
1. ✅ Check plan file exists: `ls docs/plans/plan.{seq}.*.md`
2. ✅ Verify filename format: `plan.{seq}.{extra}.md`
3. ✅ Create plan from template if missing
...
**Solutions**:
1. ✅ Re-run `zco_claude_init.py` to recreate links
2. ✅ Check source directories exist in parent project
